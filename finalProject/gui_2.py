...
PLAYER2_RED = (255, 0, 0)
PLAYER2_RED_DEST = (255, 100, 100)

PLAYER1_GREEN = (0, 200, 0)
PLAYER1_GREEN_DEST = (100, 200, 100)
...
def display_layout(display, cannonical):
    ...
    for pit in range(0, 36):
        if pit in newLines:
            color_circle(display, cannonical, pit, set(), x_coord, y_coord)
            rowCounter += 1
...
def draw_board(display, game, cannonical, player_turn, highlighted, selected_pit):
    display.fill(BACKGROUND)
    pg.draw.circle(display, GAME_BOARD_UNDER, (WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2), (GAME_BOARD_RADIUS + 5), 0)
    pg.draw.circle(display, GAME_BOARD, (WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2), GAME_BOARD_RADIUS, 0)
    display_title(display)
    ...
    for pit in range(0, 36):
        if pit in newLines:
            color_circle(display, cannonical, pit, highlighted_moves, x_coord, y_coord)
            rowCounter += 1
            y_coord += CIRCLE_DIAMETER + V_SPACING
            if rowCounter < 6:
                x_coord -= rowCounter * (CIRCLE_DIAMETER + H_SPACING) - 25
    ...
    if pg.mouse.get_pressed()[0]:
        highlighted_moves, selected_pit = highlight_selected_moves(game, display, player_turn, selected_pit, highlighted_moves)
...
def highlight_selected_moves(game, display, pturn, selected_pit, highlighted_moves):
    # Get the mouse position
    mouse_pos = pg.mouse.get_pos()
    # Gets possible moves for given player's turn (0 if 1, 1 if 2)
    possibleMoves = game.get_legal_moves(pturn)
    ...
    for pit in range(0, 36):
        ...
        if (math.sqrt(sqx + sqy)) < CIRCLE_RADIUS**2:
            ...
            player_piece = -1
            for i in range(0, len(game.pieces)):
                if pit == game.pieces[i]:
                    player_piece = i
                    # print(f"Found player {player_turn} piece {i} at index {pit}")
                    break
            if player_piece == -1:
                color_new_circle(game, pit, display, pturn, highlighted_moves, x_coord, y_coord)
            if player_piece != -1:  # if clicked pit is a player piece
                if selected_pit != pit:  # If the pit is not the same as the previously selected
                    for highlighted in highlighted_moves:  # Highlight new moves
                        ...
                        color_circle(display, game, pit, highlighted_moves, x_coord, y_coord)
                else:
                    selected_pit = pit
                    highlighted_moves = set(possibleMoves[player_piece])  # TODO NEED TO TEST GUI ON HIGHLIGHTED MOVES!!
                    highlight_potential_moves(highlighted_moves, player_piece, display)

    return highlighted_moves, selected_pit

def color_new_circle(board, pit_index, display_surface, player_turn, highlighted_moves, x_coord, y_coord):
    ...
    elif board[pit_index] == -1:
        pg.draw.circle(display_surface, PLAYER2_RED, (x_coord, y_coord), CIRCLE_RADIUS, 0)
    else:
        pg.draw.circle(display_surface, HIGHLIGHT if pit_index in list(highlighted_moves) else EMPTY_CELL,
                       (x_coord, y_coord), CIRCLE_RADIUS, 0)

def color_circle(display_surface, board, pit_index, highlighted_moves, x_coord, y_coord):
